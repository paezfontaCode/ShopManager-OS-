...
import { mockWorkOrders, mockParts } from '../data/mockRepairData';
...
const RepairDashboard: React.FC = () => {
    ...
    const pendingCount = mockWorkOrders.filter(o => ['Recibido', 'En Diagnóstico'].includes(o.status)).length;
    const inProgressCount = mockWorkOrders.filter(o => o.status === 'En Reparación' || o.status === 'Esperando Parte').length;
    const readyCount = mockWorkOrders.filter(o => o.status === 'Reparado').length;
    const lowStockCount = mockParts.filter(p => p.stock < 5).length;
    ...
    return (
        <div>
            ...
            <div className="mt-8 bg-white dark:bg-dark-card p-4 rounded-lg shadow-md">
                ...
                <RecentWorkOrdersTable orders={mockWorkOrders} />
            </div>
        </div>
    );
};
